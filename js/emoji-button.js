/**
 *
 * @type {{calculatorItem: *, triggerAfterValidate: *, buttonSubmitForm: *}}
 */
let edm = (function ($, window, undefined) {
    'use strict';
...
var E__source_emojiButton_node_modules_tabbable = tabbable;
...
function focusTrap(element, userOptions) {
  ...
  function checkPointerDown(e) {
    ...
    if (config.clickOutsideDeactivates) {
      deactivate({
        returnFocus: !E__source_emojiButton_node_modules_tabbable.isFocusable(e.target)
      });
      return;
    }
  ...
  function updateTabbableNodes() {
    var tabbableNodes = E__source_emojiButton_node_modules_tabbable(container);
...
class EmojiPreview {
    ...
    showPreview(emoji) {
        ...
        if (emoji.custom) {
            ...
            content = twemoji_npm.parse(emoji.emoji, twemojiOptions);
        }
...
class Emoji {
    ...
    */
        if (this.emoji.custom) {
            ...
            content = this.lazy ? smile : twemoji_npm.parse(this.emoji.emoji);
        }
    ...
    updateRecents() {
        if (this.options.showRecents) {
            ...
            if (recentsContainer && recentsContainer.parentNode) {
                recentsContainer.parentNode.replaceChild(new EmojiContainer(emojiCategories.recents, true, this.events, this.options, false).render(), recentsContainer);
            }
        }
    }
    render() {
...
class EmojiButton {
    ...
    observeForLazyLoad() {
            ...
            visibleElements.forEach(element => {
                if (!element.dataset.loaded) {
                    if (element.dataset.custom) {
                        ...
                        element.innerHTML = twemoji_npm.parse(element.dataset.emoji, twemojiOptions$1);
                        element.dataset.loaded = true;
                        element.style.opacity = '1';
                    }
                }
        ...
        const emojiElements = this.emojiArea.emojis.querySelectorAll(`.${CLASS_EMOJI}`);
    ...
    hidePicker() {
        ...
        setTimeout(() => {
            ...
            const searchField = this.pickerEl.querySelector(`.${CLASS_SEARCH_FIELD}`);
            if (searchField) {
                searchField.value = '';
            }
            const variantOverlay = this.pickerEl.querySelector(`.${CLASS_VARIANT_OVERLAY}`);
            if (variantOverlay) {
                this.events.emit(HIDE_VARIANT_POPUP);
            }
